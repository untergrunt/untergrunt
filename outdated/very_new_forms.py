...
screen=TK.Label(app,text=(H)*((W)*'#'+'\n'),font=('Courier New',charsize),bg='black',fg='white',borderwidth=0)
...
def text(x,y,text,fg='white',bg='black',font=CHARFONT,size=charsize,align='nw'):
  if graph_mode=='symbols':
    s=String(screen['text'])
    screen['text']=screen['text'][:y*H+x]+text+screen['text'][y*H+x+len(text):]
...
def symbol(x,y,txt,color='white'):
  if graph_mode=='symbols':
    ...
    #print(str(x)+' '+str(y))
    #screen[x][y]['fg']=color
    screen['text']=screen[:y*H+x]+'txt'+screen[y*H+1+x:]
  elif graph_mode=='window':
    global scr
